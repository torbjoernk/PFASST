    ...
    namespace heat3d
    {
      ...
      Heat3D<SweeperTrait, Enabled>::init_opts()
      {
        ...
        config::options::add_option<spatial_t>("Heat 3D", "nu",
                                               "thermal diffusivity");
        config::options::add_option<size_t>("Heat 3D", "np_space",
                                            "number processes per spatial block");
      }
      ...
      template<class SweeperTrait, typename Enabled>
      shared_ptr<typename SweeperTrait::spatial_comm_t>&
      Heat3D<SweeperTrait, Enabled>::spatial_communicator()
      {
        return this->_space_comm;
      }

      template<class SweeperTrait, typename Enabled>
      const shared_ptr<typename SweeperTrait::spatial_comm_t>&
      Heat3D<SweeperTrait, Enabled>::get_spatial_communicator() const
      {
        return this->_space_comm;
      }
