...
using VectorEncapTrait = pfasst::vector_encap_traits<double, double, 2>;
...
INSTANTIATE_TYPED_TEST_CASE_P(Spectral2DTransfer, Concepts, SpectralTransferTypes);
  ...
  : public ::testing::Test
{
  protected:
    ...
    typedef pfasst::encap::VectorEncapsulation<double, double, 2>          encap_type;
    ...
    virtual void SetUp()
    {
      this->coarse_encap = make_shared<encap_type>(vector<double>(4, 1.0));
      this->fine_encap = make_shared<encap_type>(16);

      ASSERT_THAT(this->coarse_encap->get_dimwise_num_dofs(), Pointwise(Eq(), array<int, 2>{2, 2}));
      ASSERT_THAT(this->fine_encap->get_dimwise_num_dofs(), Pointwise(Eq(), array<int, 2>{4, 4}));
    }
};
...
TEST_F(Interpolation, interpolate_constant)
{
  ...
  EXPECT_THAT(this->fine_encap->data(), Pointwise(Eq(), vector<double>(16, 1.0)));
}
